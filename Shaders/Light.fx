...
float4 ComputeToonDefferedLight(float4 albedo, float3 normal, float2 uv, float3 worldPosition)
{
    float4 ambientColor = 0;
    float4 diffuseColor = 0;
    float4 specularColor = 0;
    ...
    // Ambient
    // Material.ambient가 (1,1,1,1)인지 꼭 확인
    ambientColor = texColor * GlobalLight.ambient;
    ...
    diffuseColor = texColor * threeStep * GlobalLight.diffuse;
    ...
    // 최종 스펙큘러 컬러
    specularColor = GlobalLight.specular * toonSpec;
    //specularColor = GlobalLight.specular * Material.specular * specFactor;
