    ...
    _In_ int       nCmdShow)
{
    ...
    unique_ptr<ToolApp>        pToolApp = ToolApp::Create();
    if (nullptr == pToolApp)
        return FALSE;

    if (FAILED(GameInstance::Get().Add_Timer(TEXT("Timer_Default"))))
        return FALSE;
    if (FAILED(GameInstance::Get().Add_Timer(TEXT("Timer_60"))))
        return FALSE;
    ...
    while (true)
    {
        ...
        GameInstance::Get().Compute_TimeDelta(TEXT("Timer_Default"));

        fTimeAcc += GameInstance::Get().Get_TimeDelta(TEXT("Timer_Default"));
